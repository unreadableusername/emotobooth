...
const redisPromises = Promise.promisifyAll(redis, { suffix: 'Promise' });
...
const sessionImages = {};
let sessionId = 1;
const sessionIsComplete = false;
const SocialPublisher = require('./scripts/socialPublisher');
...
let dontPrint = false;
const socialConfig = ['Thanks for visiting the @GCPEmotobooth! See all photos and data from this session → ', 'Emotobooth'];
let socialPublisher;
...
// Get social config
const regex_tweet = new RegExp('^.*' + 'TWEET="' + '.*$', 'm');
const regex_event_name = new RegExp('^.*' + 'EVENT_NAME="' + '.*$', 'm');

fs.exists(CONFIG_PATH, (fileok) => {
  if (fileok) {
    fs.readFile(CONFIG_PATH, 'utf8', (err, data) => {
      const t = regex_tweet.exec(data);
      const e = regex_event_name.exec(data);
...
const checkDirs = [config.inDir, config.outDir, config.printDir, config.photostripDir];
...
// Redis
const client = redisPromises.createClient(config.redis.port, config.redis.host);
client.hkeys('image-data', (err, replies) => {
...
function callNextJobs(jobName, data) {
  const outEvents = jobMapping[jobName].out;
...
function trackInputJobs(jobName, jobID) {
  ...
  const inEvents = jobMapping[jobName].in;
  const numInEvents = jobMapping[jobName].in.length;
  const numInputEvents = Hash(jobMapping).filter((value, key) =>
    // jobs whose outs plug into our ins ;)
    _.intersection(value.out, inEvents).length > 0).keys.length;

  // This job was called by another, so increment the counter
  const redisKey = sprintf('%s-%s', jobID, jobName);
  return client.hincrbyPromise('finished-jobs', redisKey, 1)
    .then((numFinishedEvents) => {
      logger.debug(sprintf(
        'Job %s received %d/%d events',
...
function sessionComplete(job, finish) {
  const currSessionId = job.sessionId || sessionId;
  sessionId++;
  let sess = sessionImages[currSessionId];
  if (sess) {
    if (!sess.kill) {
...
function processNewImage(imagePath, imageData) {
  ...
  const out = path.join(config.outDir, sprintf('%s-orig.jpg', imageData.id));
...
function topThreeImages(sess) {
  ...
  const topKeys = keys.sort((a, b) => b[1] - a[1]).slice(0, 3);
...
function getVisionData(job, finish) {
  fs.readFile(job.data.path, (err, data) => {
    ...
    const encodedDataShort = data.toString('base64').substring(0, 200);
  ...
  if (!incompleteSession) {
    ...
    for (const key in sess) {
      if (sess[key].wasProcessed) {
        if (sess[key].score > 1) {
          scoredPhotos.push({ key, score: sess[key].score });
        }
      }
    }
    ...
    for (const key in sess) {
      ...
      if (sess[key].wasProcessed) {
        if (sess[key].score <= 1) {
          const scoreNum = sess[key].score;
          scoredPhotos.push({ key, score: scoreNum });
        }
      }
    }
    ...
    const out = path.join(config.outDir, sprintf('%s-final.jpg', sess[incompleteSession].id));
    sess[incompleteSession].finalPathChrome = out;

    const outChromeless = path.join(config.outDir, sprintf('%s-final-chromeless.jpg', sess[incompleteSession].id));
    ...
    const outJs = path.join(config.outDir, sprintf('%s.js', sess[incompleteSession].id));
...
app.get('/history-data', (req, res) => {
  client.hkeys('image-data', (err, replies) => {
    if (replies.length) {
      ...
      client.hmget('image-data', filteredReplies, (e, r) => {
        res.setHeader('Content-Type', 'application/json');
        r.forEach((image, i) => JSON.parse(image));
...
router.route('/images')
  .get((req, res) => {
    let images = fs.readdirSync(config.outDir);
    images = images.filter(filename => filename.endsWith('.json'));
    res.json({ images });
  });

router.route('/images/:image_id')
  .get((req, res) => {
    const jsonFilename = `${req.params.image_id}.json`;
    const jsonPath = path.join(config.outDir, jsonFilename);
